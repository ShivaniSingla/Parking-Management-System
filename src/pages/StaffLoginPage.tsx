...
const StaffLoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Attempting staff sign in...");
      ...
      if (data.user) {
        console.log("Sign in success, verifying staff profile...");
        ...
        if (profileErr || !profile) {
          console.log("Staff profile missing, creating now...");
          const { error: insertErr } = await supabase.from('profiles').insert([
            { id: data.user.id, username: data.user.email || 'staff', role: 'staff' }
          ]);
          if (!insertErr) {
  ...
  return (
    <div className="login-page">
      <div className="login-card">
        <div className="login-logo" style={{ backgroundColor: 'var(--success)', color: 'white', position: 'relative' }}>
          <FaUserTie />
          <div 
